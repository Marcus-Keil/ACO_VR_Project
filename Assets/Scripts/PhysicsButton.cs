...
public class PhysicsButton : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Vector3.Distance(buttonTop.position, buttonLowerLimit.position) < upperLowerDiff * threshHold)
            isPressed = true;
        else if (Vector3.Distance(buttonTop.position, buttonLowerLimit.position) > upperLowerDiff * (1 - threshHold))
            isPressed = false;
    ...
    void Pressed()
    {
        ...
        pressedSound.pitch = Random.Range(0.8f, 1.0f);
        pressedSound.Play();
        onPressed.Invoke();
    }
    ...
    void Released()
    {
        ...
        pressedSound.pitch = Random.Range(1.1f, 1.3f);
        pressedSound.Play();
        onReleased.Invoke();
    }
}
